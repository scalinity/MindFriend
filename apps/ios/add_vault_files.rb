#!/usr/bin/env ruby
require 'xcodeproj'
require 'pathname'

project = Xcodeproj::Project.open('MindFriendApp.xcodeproj')
app_target = project.targets.find { |t| t.name == 'MindFriendApp' }
test_target = project.targets.find { |t| t.name == 'MindFriendAppTests' }

def find_or_create_group(project, file_path)
  components = Pathname.new(file_path).each_filename.to_a
  filename = components.pop

  current_group = project.main_group
  components.each do |component|
    child = current_group.children.find { |c|
      c.is_a?(Xcodeproj::Project::Object::PBXGroup) && c.name == component
    }
    current_group = child || current_group.new_group(component, component)
  end

  [current_group, filename]
end

# Files to add to MindFriendApp target
app_files = [
  "MindFriendApp/Core/Models/VaultModels.swift",
  "MindFriendApp/Core/Services/VaultEncryptionService.swift",
  "MindFriendApp/Core/Services/VaultAuthService.swift",
  "MindFriendApp/Core/Services/VaultStorageService.swift",
  "MindFriendApp/Features/Vault/VaultViewModel.swift",
  "MindFriendApp/Features/Vault/VaultListView.swift",
  "MindFriendApp/Features/Vault/VaultEntryEditorView.swift",
  "MindFriendApp/Features/Vault/VaultSettingsSection.swift"
]

# Files to add to MindFriendAppTests target
test_files = [
  "MindFriendAppTests/VaultEncryptionTests.swift",
  "MindFriendAppTests/VaultStorageTests.swift"
]

# Add app files
app_files.each do |file_path|
  basename = File.basename(file_path)

  # Check if file already exists in project
  existing = project.files.find { |f| File.basename(f.path.to_s) == basename }
  if existing
    puts "Skipping (already exists): #{file_path}"
    next
  end

  group, filename = find_or_create_group(project, file_path)
  file_ref = group.new_reference(filename)
  file_ref.source_tree = '<group>'
  file_ref.last_known_file_type = 'sourcecode.swift'
  app_target.source_build_phase.add_file_reference(file_ref)

  puts "Added to MindFriendApp: #{file_path}"
end

# Add test files
test_files.each do |file_path|
  basename = File.basename(file_path)

  # Check if file already exists in project
  existing = project.files.find { |f| File.basename(f.path.to_s) == basename }
  if existing
    puts "Skipping (already exists): #{file_path}"
    next
  end

  group, filename = find_or_create_group(project, file_path)
  file_ref = group.new_reference(filename)
  file_ref.source_tree = '<group>'
  file_ref.last_known_file_type = 'sourcecode.swift'
  test_target.source_build_phase.add_file_reference(file_ref)

  puts "Added to MindFriendAppTests: #{file_path}"
end

project.save
puts "\nProject saved successfully!"
