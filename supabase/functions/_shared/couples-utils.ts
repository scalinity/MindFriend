...
export async function userHasPremiumAccess(
  ...
  if (subscriptionError) {
    throw new Error(`Failed to check user subscription: ${subscriptionError.message}`);
  }
  ...
  if (partnershipError) {
    throw new Error(`Failed to check partner link: ${partnershipError.message}`);
  }
  ...
  const { data: partnerSubscription, error: partnerSubscriptionError } = await supabase
    .from("subscriptions")
    .select("status")
    .eq("user_id", partnerId)
    .maybeSingle();

  if (partnerSubscriptionError) {
    throw new Error(`Failed to check partner subscription: ${partnerSubscriptionError.message}`);
  }
...
export async function isPartnerWith(
  ...
  if (error) {
    throw new Error(`Failed to check partnership: ${error.message}`);
  }
...
export async function getPartnerId(
  ...
  if (error) {
    throw new Error(`Failed to get partner ID: ${error.message}`);
  }
