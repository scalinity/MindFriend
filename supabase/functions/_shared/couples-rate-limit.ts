...
export async function checkRateLimit(
  ...
  try {
    ...
    if (error) {
      ...
      return true; // Fail CLOSED on database error - deny request when system is degraded
    }
    ...
    return true; // Fail CLOSED on unexpected error - deny request when system is degraded
  }
}
