...
import { QUEST_ARC_CONSTANTS } from "../_shared/quest-arc-constants.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    if (arc.status === "paused") {
      ...
      expiresAt.setDate(expiresAt.getDate() + QUEST_ARC_CONSTANTS.PAUSE_EXPIRATION_DAYS);
    ...
    if (updateError) {
      console.error("Error pausing arc:", { code: updateError.code });
    ...
    console.error("pause-quest-arc error:", { message: (error as Error).message });
