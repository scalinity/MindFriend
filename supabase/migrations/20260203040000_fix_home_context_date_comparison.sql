-- Fix "operator does not exist: date = text" in get_home_context
-- moods.local_date is DATE type, but was compared with CURRENT_DATE::TEXT
-- quests.local_date is TEXT type, so CURRENT_DATE::TEXT is correct there
...
BEGIN
    ...
    WITH
    -- Get today's mood (moods.local_date is DATE, use CURRENT_DATE not ::TEXT)
    today_mood AS (
        SELECT mood_score
        FROM moods
