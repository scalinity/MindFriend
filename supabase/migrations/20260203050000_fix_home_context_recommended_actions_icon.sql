-- Fix HomeContext decoding error: "The data couldn't be read because it is missing"
-- The recommended_actions JSON objects were missing the 'icon' field
-- which is required by the Swift RecommendedAction struct
...
BEGIN
    ...
    WITH
    -- Get today's mood (moods.local_date is DATE, compare with CURRENT_DATE)
    today_mood AS (
        SELECT mood_score
        FROM moods
    ...
    -- Quest completed today (quests.local_date is TEXT, use CURRENT_DATE::TEXT)
    quest_status AS (
        SELECT EXISTS(
        ...
        'recommended_actions', (
            ...
            FROM (
                SELECT jsonb_build_object(
                    ...
                    'icon', 'star.fill',
                    'priority', 1
                ) AS action, 1 AS priority
                WHERE NOT cd.quest_done
                UNION ALL
                SELECT jsonb_build_object(
                    'type', 'exercise',
                    'title', 'Try a quick exercise',
