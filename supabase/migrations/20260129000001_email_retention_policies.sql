...
GRANT EXECUTE ON FUNCTION pg_catalog.pg_cron_schedule(text, text, text) TO postgres;
...
SELECT cron.schedule(
  ...
  '0 4 * * *',  -- 4 AM daily
  $$DELETE FROM email_dead_letter_queue 
    WHERE moved_to_dlq_at < NOW() - INTERVAL '30 days'$$
);
...
SELECT cron.schedule(
  ...
  $$DELETE FROM email_queue 
    ...
    AND retry_count >= 3$$  -- Only delete if exhausted retries
);
...
-- Create index on retention_until for efficient cleanup queries
