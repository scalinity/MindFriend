...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_pathways') THEN
        RAISE NOTICE 'user_pathways table does not exist, skipping migration';
        RETURN;
    END IF;
