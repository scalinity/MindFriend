-- Fix subscription schema mismatches in quota enforcement
-- BUG: Was using s.tier (doesn't exist) instead of s.plan_type
-- BUG: Was checking for 'trialing' status (doesn't exist)
-- BUG: Not handling NULL expires_at (lifetime subscriptions)

-- Fix enforce_capsule_quota trigger function
...
BEGIN
    ...
    -- FIX: Changed s.tier to s.plan_type (correct column name)
    -- FIX: All paid plan types (individual, couples, family, organization) get premium features
    -- FIX: Handle NULL expires_at (lifetime subscriptions) with COALESCE
    SELECT
        COALESCE(
            s.plan_type IN ('individual', 'couples', 'family', 'organization')
            AND s.status = 'active'
...
-- Fix check_capsule_quota RPC function
...
BEGIN
    ...
    -- FIX: Changed s.tier to s.plan_type
    -- FIX: All paid plan types get premium features
    -- FIX: Handle NULL expires_at for lifetime subscriptions
    SELECT
        COALESCE(
            s.plan_type IN ('individual', 'couples', 'family', 'organization')
            AND s.status = 'active'
...
'Trigger function that atomically enforces capsule quota before insert. Fixed to use plan_type column and handle lifetime subscriptions.';

COMMENT ON FUNCTION check_capsule_quota(UUID, INTEGER) IS
'Pre-flight quota check for user-friendly error messages. Fixed to use plan_type column and handle lifetime subscriptions.';
