...
-- Note: email tables created in later migration (20260420000000_email_summary_schema.sql)
DO $$
BEGIN
  IF EXISTS (
...
-- Attach audit trigger to email_logs (only if table exists)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables
  ...
  ELSE
    RAISE NOTICE 'email_logs table does not exist yet, skipping trigger creation';
  END IF;
END $$;

-- 6. Add metadata columns for GDPR tracking (only if tables exist)
DO $$
BEGIN
  ...
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'email_dead_letter_queue') THEN
